...
async function showCrud(req, res, next) {
    try {
        ...
        const { rows, columns, total, primaryKey: pk } = await svc.getTableRows(table, page, 10, search, sort, direction);
        ...
        res.render('crud', {
            title: svc.toTitleCase(table), tables, table, columns, rows, pk,
            total, totalPages, page, search, sort, direction, fkOptions,
            viewRow, editRow,
            created: 'created' in req.query,
            updated: 'updated' in req.query,
            deleted: 'deleted' in req.query,
            currentPath: '/crud', currentTable: table,
...
async function deleteRecord(req, res, next) {
    try {
        const table = req.query.table || '';
        await svc.deleteRow(table, req.query.id);
